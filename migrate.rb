...
#  Turkish edited and fixed by Zafer Zent github.com/zaferzent
#  * Post slug turkish character solved
#  * Post date update/published solved
#  * Feed post limit change this label
...
def parse_post_entries(feed, verbose)
  ...
  feed.entries.each do |post|
    obj = Hash.new
    created_datetime = post.published  #updated published olarak değiştirildi, çünkü bize ilk yayınlanma zamanı gerekiyor.  
	creation_date = Date.parse(created_datetime.to_s)

    title = post.title
	atitle = title.downcase.strip.gsub(' ', '-').gsub('ş','s').gsub('ü','u').gsub('ğ','g').gsub('ı','i').gsub('ç','c').gsub('ö','o').gsub(/[^\w-]/, '')

    file_name = creation_date.to_s + "-" + atitle.split(/  */).join("-").delete('\/') + ".html"
...
def main
  ...
  xml = HTTParty.get("https://www.zaferzent.com/feeds/posts/default/-/Blogger?orderby=updated&max-results=9999").body
